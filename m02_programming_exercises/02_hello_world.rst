...
10. run.sh line ``capture python3 /exercise/tests.py`` is executed. Because the
...
13. Now the Python intepreter is called with the parameter ``/exercise/tests.py``.
    The current working directory is still ``/submission/user``.

14. Inside **tests.py**, the main level script is executed:
    ...
    <https://github.com/Aalto-LeTech/python-grader-utils>`_, which is included
    in the apluslms/grade-python container. The class *PointsTestRunner* is
    in the file `graderunittest.py <https://github.com/Aalto-LeTech/python-grader-utils/blob/master/graderutils/graderunittest.py>`_.
    For the details, Graderutils is inside the grade-python container at
    ``/usr/local/lib/python3.5/dist-packages/graderutils``.
...
15. The Python module *graderutils* runs a *PointsTestRunner* instance. First it
    ...
    class ``TestHelloPython`` in the **tests.py**. These test methods are
...
17. Because ``test_function`` has the docstring
    ``"""Check hello function exists (1p)"""``, Graderutils will print this
    *and* count one point if the test passes. Similarly, ``test_import`` also
    yields one point if it passes. ``test_return`` will yield three points if
    it passes, because its docstring ends with ``(3p)``.
